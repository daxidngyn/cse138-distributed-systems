...
class TestShard(unittest.TestCase):
    ...
    def test_redistribute_shards_3(self):
        ''' view has 7 elements, redistribute to 3 shards '''
        view = ['10.10.0.2:8090', '10.10.0.3:8090', '10.10.0.4:8090', '10.10.0.5:8090', 
                '10.10.0.6:8090', '10.10.0.7:8090', '10.10.0.8:8090']

        kserver = Server("test_redistribute_shards")

        shards = kserver.shard._redistribute_shards(3, view)

        self.assertEqual(len(shards), 3)
        self.assertIn('alligator', shards)
        self.assertIn('buffalo', shards)
        self.assertIn('cat', shards)
        set1 = shards['alligator']
        set2 = shards['buffalo']
        for a in set1:
            self.assertTrue(a not in set2)
        ...
        total_nodes = [ x for l in list(shards.values()) for x in l ]
        self.assertEqual(len(total_nodes), len(view))
